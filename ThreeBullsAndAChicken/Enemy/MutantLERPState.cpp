...
{
	m_startingPos = mutant->getNode()->getPosition();
...
void MutantLERPState::update(MutantModelHandler* mutant)
{
	...
	if (m_player)
	{
		if (m_lerpState == LERP_STATE::LERP_WALK)
			m_lerpState = mutant->lerpWalk(m_startingPos, m_player->getNode()->getPosition(), dt/5.0);
		else if (m_lerpState == LERP_STATE::LERP_ATTACK)
		{
			m_lerpState = mutant->lerpAttack(m_startingPos, m_player->getNode()->getPosition(), dt/5.0);
			if (m_lerpState == LERP_STATE::LERP_WALK)
			{
				cout << "attackDone" << endl;
				mutant->resetLerp();
