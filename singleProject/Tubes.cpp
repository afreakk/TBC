...
namespace Ogre
{
	...
	SeriesOfTubes::SeriesOfTubes(
		...
		const Ogre::Vector3& offset /* ZERO */)
		: mSceneMgr(sceneMgr),
	...
	void SeriesOfTubes::addPoint(const Ogre::Node* pos)
	{
		mLineVertices.push_back(pos);
	}

	void SeriesOfTubes::updateLatestPoint(const Ogre::Node* node)
	{
		mLineVertices.back() = node;
	}
	void SeriesOfTubes::removePoint(const Ogre::uint pointNumber)
	{
		if (pointNumber < mLineVertices.size())
		{
			std::vector<const Ogre::Node*>::iterator it = mLineVertices.begin() + pointNumber;
...
//			pEnt->setRenderingDistance(mSphereMaxVisDistance);
			...
			if (mSceneNode)
			{
				...
				pChildNode->setPosition(((*it)->getPosition()+m_offset));
	...
	Ogre::ManualObject* SeriesOfTubes::createDebug(const Ogre::String& name)
	{
		...
		std::vector<const Ogre::Node*>::iterator it = mLineVertices.begin();
		for (; it != mLineVertices.end(); ++it)
		{
			pObj->position((*it)->getPosition()+m_offset);
